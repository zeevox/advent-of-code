...
def main(parttwo = False):
    ...
    for passport in passports:
        ...
        if parttwo and re.match(r"(?=.*byr:(19[2-9][0-9]|200[1-2]))(?=.*iyr:(201[0-9]|2020))(?=.*eyr:(202[0-9]|2030))(?=.*hgt:(1[5-8][0-9]cm|19[0-3]cm|59in|6[0-9]in|7[0-6]in))(?=.*hcl:#(?:[0-9a-fA-F]{3}){1,2})(?=.*ecl:(amb|blu|brn|gry|grn|hzl|oth))(?=.*pid:\d{9})", passport) is None:
            continue
        if parttwo: print(passport)
        valid += 1
    return valid
