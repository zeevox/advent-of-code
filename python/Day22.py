...
def parse(inp):
    ...
    for player in inp.strip().split("\n\n"):
        deck = []
        for card in player.splitlines()[1:]:
...
def combat_round(deck1, deck2, game, out = False, game_count = 1):
    ...
    # otherwise, this round's cards must be in a new configuration
        ...
        if game is not None and card1 <= len(deck1) and card2 <= len(deck2):
            if out: print("Playing a sub-game to determine the winner...")
            winner = play_game(deck1[:card1], deck2[:card2], False, out, game_count + 1)

        if out: print(f"Player {winner} wins this round of game {game_count}")
        if winner == 1:
            deck1.append(card1)
            deck1.append(card2)
        else:
